...
import TakeNote from '../../models/Course/TakeNote.js';
...
class TakeNoteController {
    // [GET] /takenotes
    async getAllTakeNotes(req, res, next) {
        ...
        try {
            const takenotes = await TakeNote.find(query).sort({ updatedAt: order || -1 }).populate('')
            const totalTakeNotes = await TakeNote.find(query).countDocuments()
            res.json({
                takenotes,
                totalTakeNotes
            });
    ...
    // [POST] /takenotes
    async addTakeNote(req, res, next) {
        try {
            const { author, content } = req.body;


            const newTakeNote = new TakeNote({
                author,
                content
            })
            await newTakeNote.save();

            req.io.emit('lesson:create', newTakeNote);
            res.json(newTakeNote);
        } catch (error) {
            next(error);
        }
    }
...
export default new TakeNoteController();
